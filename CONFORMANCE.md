...
`<textarea>`/`<title>` body containing markup **the libxml2 way**: measured here, tokens
`['a','b','Keep']`, arity 3, block id `d55c2b5c806bde213b0637da096c6b36` — byte-identical to
langsys-php's measurement and *not* the parse5 tree the fleet's fixture records for the JS family
(`['Keep','a <b>b</b>']`, arity 2). Found by the Reviewer. It is the same class as the `<noscript>`
...
the Reviewer's prompting. No rule id covers the `<Phrase>` encoding; `custom_id` rules do not apply to it, since it keys by string and never computes one |
