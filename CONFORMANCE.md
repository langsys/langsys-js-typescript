...
| SNAP-2 | not implemented | - | No snapshot loader. `LangsysApp.seedCatalog` is the seed input a snapshot would load through (SRV-4). |
| SNAP-3 | not implemented | - | No snapshot loader, so nothing yet reads a snapshot to hold it as a cache. |
...
1. **Rules not yet implemented, or partly.** MARK-2's whole-host registration; MARK-3's adoption of a stamped id and registration of a bare declaration; TOK-6's single-text-node search, which still counts text inside an excluded element or a nested phrase host; REG-10's skipped write, which `registerContentBlock` still reports as `{status:true}`; SNAP-2 and SNAP-3. Each is graded in its row.
