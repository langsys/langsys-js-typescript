...
| **Suite** | 828 tests in 47 files, `npm test`, counted at the tip of this branch |
...
Five properties this SDK commits to that no rule id names. They are graded on the same terms, but
...
| Discovery limited to the base locale | implemented | n/a (pure) | `base-locale-gate`. A project setting delivered in the handshake — `authorize-project` `data` beside `key_type`, and the catalog envelope beside `words` — read from both, re-read on every fetch, with no client override. While on, a miss is recorded only when the loaded locale IS the base locale, gating the registration queue and the discovery hint together. Absent or non-boolean means off, and the module default is off, asserted on a FRESH module instance because every other case resets the store first. The catalog cache never carries it: it is a top-level sibling of `data` on both catalog routes, which Langsys confirmed from the backend's own response composition. Raised by the Laravel lane and measured here — without it a write-enabled session browsing in Spanish filed Spanish sentences as source phrases, and a read-only one hinted a localized URL. Wire name `discovery_base_locale_only`, agreed with Langsys. **The field is not on the wire yet**: the spec rule and the backend are both pending, the backend on the 907 branch, so every session today reads it as absent, which is off. Both handshakes are driven by doubles here, which is what this row's evidence rests on until a server sends the field. Mutations, in an isolated copy: no gate on the miss path (2 red); the envelope not read (3); the authorize body not read (1); an absent field read as on (1); the store defaulting to on (1). |
| Text a producer marked resolved is never recorded | implemented | n/a (pure) | `resolved-marker`. `data-ls-resolved` on any ancestor, `data-langsys-resolved` accepted on read (MARK-2), the value optional and informational, `="false"` and `="0"` opting a subtree back out under the marker convention both SDKs already share. Inherited, nearest ancestor deciding, because text a server printed inline has no element of its own to carry a mark. It suppresses both lanes on the `<Translate>` single-token path, on content-block registration and on `<Phrase>`, and says nothing about identity: a block inside a resolved scope still stamps the id the tokenizer derives, and a translation the catalog holds is still rendered. Shape agreed with the PHP and Laravel lanes against their own measurements. Mutations, in an isolated copy: the single-token path ignores it (5 red); block registration ignores it (1); `Phrase` ignores it (1); not inherited (7); canonical spelling only (1); the opt-out ignored (2). |

**Why server messages are not the exposure (MSG-5).** An entry is rendered by
`t(entry.template, category, entry.params)` and `message` is never a lookup key, so the
templates a server SDK emits arrive as source text and register correctly. What the two rows
above address is ordinary page output — `t()`, Blade `@t`, fill-mode `__()` — on pages that
also run a JS SDK. MSG-5 lives at spec HEAD `70320628`, blob `f8ff6e1e` (specVersion 8.1.0);
this file is pinned to 8.0.1 and has not been re-audited against that revision, so the family
is cross-referenced here rather than rowed. Langsys is writing spec rules for both the gate and
the marker, each pending their own operator's word; until those land, these two properties sit
here rather than under rule ids. The producer half of the marker — who writes
`data-ls-resolved` — belongs to the server lanes and the 907 branch, not to this one.
