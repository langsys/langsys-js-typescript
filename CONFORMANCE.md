...
| **specVersion** | 8.2.15 |
| **Spec revision read** | langsys2 f5568b88…, docs/sdk-spec.mdx blob b9fd4b5b1c15f7ba29656d550dca1f06013327c0 (specVersion 8.2.15). Re-derived with `git -C ~/Documents/dev/langsys2 ls-tree f5568b88 docs/sdk-spec.mdx` at this write and checked by `npm run verify:spec`, which also fails if the vector file cites a different blob. Every rule id is counted by `npm run tally:conformance`. |
| **SDK revision** | `feature/838_write_key_gating_reland`, cut from `origin/main` `2d7b11f` (v0.6.5) |
| **Suite** | 1216 tests in 60 files, `npm test`, counted at the tip of this branch |
...
| MSG-1 | implemented | n/a (pure) | `resolveServerMessages` finds every entry wherever it sits, and `toServerMessage` fixes the four pieces. The vector file `server-message-vectors.json` is authored here for the fleet (47 rows: 11 marker, 10 fill, 12 resolve, 10 render, 4 canonical entries from langsys4's wire examples), asserted by `server-messages` (59 tests). Its resolve rows: the langsys envelope for a system error and for a validation failure, a foreign container key, JSON:API `errors[].meta`, a top-level array, a configured `key` that narrows and one that is absent, an entry missing `template` and one with a non-string piece (neither resolves), an entry's `params` never searched, an empty `field` dropped. Also: the canonical entries render identically from the default envelope and from a `resolver` over a foreign body; a resolver's output passes the same entry check; a cyclic body terminates. Mutations, in an isolated copy: search one level deep only (8 red); `key` ignored (2); `template` optional (2); `params` searched (1); empty `field` kept (1). |
| MSG-2 | implemented | n/a (pure) | The client half: `renderServerMessage` never reads `code`, and `SERVER_MESSAGE_CODES` carries the spec's twenty-one slugs in order. Render row `code-does-not-choose-text` renders the same template under a different code to the same text; mutating the render to return `message` for one code reds it (1). The same-code-across-locales and size-code clauses are the server's, which assigns codes. The vector file `server-message-vectors.json` is authored here for the fleet (47 rows: 11 marker, 10 fill, 12 resolve, 10 render, 4 canonical entries from langsys4's wire examples), asserted by `server-messages` (59 tests). |
| MSG-5 | implemented | n/a (pure) | `renderServerMessage` renders `t(template, category, params ?? {})` when the catalog holds the template and the entry's `message` otherwise. The vector file `server-message-vectors.json` is authored here for the fleet (47 rows: 11 marker, 10 fill, 12 resolve, 10 render, 4 canonical entries from langsys4's wire examples), asserted by `server-messages` (59 tests). Its render rows: no catalog, template not translated, and a server-localised `message` kept on a miss; a translation with and without params; an ICU plural translation from a count param at 1 and 3; `message` present as a catalog key and still never looked up. Also: the lookup key is the template, spied. Mutations, in an isolated copy: `message` as the key (4 red); no fallback (1); always `message` (8). |
| MSG-6 | implemented | n/a (pure) | One category, `Errors` by default, set with `messagesCategory` in `init`. Render row `other-category-misses`: a template held under `Errors` misses under `Validation` and falls back. `server-messages` also shows the unset default renders under `Errors`, and a configured `Validation` renders there while `Errors` then misses. The vector file `server-message-vectors.json` is authored here for the fleet (47 rows: 11 marker, 10 fill, 12 resolve, 10 render, 4 canonical entries from langsys4's wire examples), asserted by `server-messages` (59 tests). Mutations, in an isolated copy: default category changed (2 red); configured category ignored (1). |
