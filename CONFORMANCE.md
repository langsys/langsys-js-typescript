...
| **specVersion** | 8.2.4 |
| **Spec revision read** | langsys2 2ddea638…, docs/sdk-spec.mdx blob 24354e2d244dc7aae15c56cb331012e29e9943b6 (specVersion 8.2.4). Re-derived with `git -C ~/Documents/dev/langsys2 ls-tree 2ddea638 docs/sdk-spec.mdx` at this write and checked by `npm run verify:spec`, which also fails if the vector file cites a different blob. Every rule id is counted by `npm run tally:conformance`. |
| **SDK revision** | `feature/838_write_key_gating_reland`, cut from `origin/main` `2d7b11f` (v0.6.5) |
| **Suite** | 903 tests in 52 files, `npm test`, counted at the tip of this branch |
...
| TOK-6 | implemented | n/a (pure) | `translate` "an attribute-only token must not take the single-token fast path" and "still uses the fast path for a genuine single TEXT token": one token that is the unit's only text node registers as a phrase, and anything else, a lone attribute included, as a content block. Attributes precede content in `tokenizer-cross-impl`. |
...
| MARK-3 | not implemented | - | The core writes `data-ls-contentblock` and never reads it: a bare marker declares no block, and an identity value is not recognised. |
| MARK-4 | not implemented | - | A phrase-marked host is excised from an enclosing walk. A content-block host is not: `<div><p>A</p><div data-ls-contentblock><p>B</p></div></div>` gives the outer unit `["A","B"]`, so `B` registers in both blocks. |
...
1. **Rules not yet implemented.** TOK-2's C0 strip; MARK-2's whole-host registration; MARK-3 and MARK-4; HINT-13; MIG-1 to MIG-8 and SNAP-2 and SNAP-3; MSG-1, MSG-2, MSG-5 and MSG-6. Each is graded in its row. Nested excision (MARK-4) moves the id of every block that contains a marked block host; fleet code nests no `<Translate>` inside another, and customer usage is not measured here.
