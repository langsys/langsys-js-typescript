...
| **Suite** | 765 tests in 43 files, `npm test`, counted at the tip of this branch |
...
2. **A write-enabled cold visit can re-register phrases before its first catalog arrives. Routed to the operator.** A miss recorded while the first catalog fetch is still in flight is flushed after the 400ms debounce, and the flush deduplicates against a catalog that is still empty. Measured in an isolated copy with write capability already granted: a miss on an already-registered phrase was POSTed when the catalog took 1s or 5s, and not when it took 300ms. No rule names the case. WIRE-4's reason, that without a catalog a miss cannot be told from a hit, covers it in spirit, and the likely fix is the one the outage path now uses: hold the flush until the first catalog settles.
...
7. **A nested content-block host is folded into the outer block. Routed to the operator.** The tokenizer does not read content-block host identity, so `<div><p>A</p><div data-ls-contentblock="…"><p>B</p></div></div>` gives the outer block `["A","B"]` (`13ac7a86…`) and the inner `["B"]` (`fb9ed17f…`), registering `B` twice, in either render order. Excising stamped hosts would move the id of every nested block. Measured blast radius in fleet code is zero: no browser binding, example or doc nests `<Translate>` inside `<Translate>`. Customer usage is unmeasured.

8. **React Native reaches no teardown flush. Routed to the operator.** `installTeardownFlush` returns early without `document`, so on React Native anything still queued when the app is killed is lost, worst under REG-8 backoff. An injected teardown signal in the `setPersistStorage` style is proposed with the React Native lane: the binding adapts the lifecycle event, and the core keeps the send.

9. **TOK-2 is held.** Its control-character clause waits on the operator's ruling on stripping C0 controls. The rest of the rule is asserted on the collapse function directly.

10. **REG-11 — the permitted suppression half is not implemented.** The warning is in and nothing is skipped, which conforms. The optional second signal — suppress when a longer catalog entry shares the prefix — would need a prefix scan of the catalog on every miss, and buys only the pollution case the warning already surfaces. Recorded as a deliberate omission rather than a gap.

11. **OBS-1's notice covers capability, not every inert state.** The write-capability warning is now above debug on both channels. HINT-9's reporting-disabled case stays debug-only deliberately — the customer chose that setting, so it is a configuration, not a fault.

12. **REG-12 — no test yet, and a redundant 32-hex guard remains** in the queue path. The primary mechanism
