...
| **specVersion** | 8.2.9 |
| **Spec revision read** | langsys2 43d090c3…, docs/sdk-spec.mdx blob 286dcfe429c8a0cdac60cdd6ab062bdf671ad078 (specVersion 8.2.9). Re-derived with `git -C ~/Documents/dev/langsys2 ls-tree 43d090c3 docs/sdk-spec.mdx` at this write and checked by `npm run verify:spec`, which also fails if the vector file cites a different blob. Every rule id is counted by `npm run tally:conformance`. |
| **SDK revision** | `feature/838_write_key_gating_reland`, cut from `origin/main` `2d7b11f` (v0.6.5) |
| **Suite** | 1080 tests in 58 files, `npm test`, counted at the tip of this branch |
...
| GATE-10 | implemented | n/a (pure) | `resolved-marker`, 14 tests: both spellings, a bare attribute, the `false`/`0` opt-out, nearest-ancestor inheritance, both lanes suppressed on the `<Translate>` single-token path, content-block registration and `<Phrase>`, and identity untouched. Mutations, each red: every reader path ignoring the marker, no inheritance, one spelling only, no opt-out. The JSON sibling-manifest clause is producer-side: this core takes no JSON prop hand-off. |
