...
| **specVersion** | 8.2.5 |
| **Spec revision read** | langsys2 b129f77d…, docs/sdk-spec.mdx blob 754daa165f5438347141de51b42a60c8b6ff5d63 (specVersion 8.2.5). Re-derived with `git -C ~/Documents/dev/langsys2 ls-tree b129f77d docs/sdk-spec.mdx` at this write and checked by `npm run verify:spec`, which also fails if the vector file cites a different blob. Every rule id is counted by `npm run tally:conformance`. |
| **SDK revision** | `feature/838_write_key_gating_reland`, cut from `origin/main` `2d7b11f` (v0.6.5) |
| **Suite** | 932 tests in 54 files, `npm test`, counted at the tip of this branch |
...
5. **React Native reaches no teardown flush.** `installTeardownFlush` returns early without `document`, so on React Native anything still queued when the app is killed is lost, worst under REG-8 backoff. The seam is ruled — an injected teardown signal in the `setPersistStorage` style, the binding adapting the lifecycle event and the core keeping the send — and not yet built.
