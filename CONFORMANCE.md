...
| **specVersion** | 8.2.10 |
| **Spec revision read** | langsys2 c1b16560…, docs/sdk-spec.mdx blob e22dad188f1c1e6a972961cdf9675a84d891f5ec (specVersion 8.2.10). Re-derived with `git -C ~/Documents/dev/langsys2 ls-tree c1b16560 docs/sdk-spec.mdx` at this write and checked by `npm run verify:spec`, which also fails if the vector file cites a different blob. Every rule id is counted by `npm run tally:conformance`. |
| **SDK revision** | `feature/838_write_key_gating_reland`, cut from `origin/main` `2d7b11f` (v0.6.5) |
| **Suite** | 1083 tests in 58 files, `npm test`, counted at the tip of this branch |
...
| MIG-1 | not implemented | - | No legacy-key mode; `mig-vectors.json` is not yet authored. |
| MIG-2 | not implemented | - | No legacy-key mode; `mig-vectors.json` is not yet authored. |
| MIG-3 | not implemented | - | No legacy-key mode; `mig-vectors.json` is not yet authored. |
| MIG-4 | not implemented | - | No legacy-key mode; `mig-vectors.json` is not yet authored. |
| MIG-5 | not implemented | - | No legacy-key mode; `mig-vectors.json` is not yet authored. |
| MIG-6 | not implemented | - | No legacy-key mode; `mig-vectors.json` is not yet authored. |
| MIG-7 | not implemented | - | No legacy-key mode; `mig-vectors.json` is not yet authored. |
| MIG-8 | not implemented | - | No legacy-key mode; `mig-vectors.json` is not yet authored. |
...
1. **Rules not yet implemented, or partly.** MARK-2's whole-host registration; MARK-3's adoption of a stamped id and registration of a bare declaration; TOK-6's single-text-node search, which still counts text inside an excluded element or a nested phrase host; REG-10's skipped write, which `registerContentBlock` still reports as `{status:true}`; MIG-1 to MIG-8, SNAP-2 and SNAP-3. Each is graded in its row.
