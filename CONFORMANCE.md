...
|---|---|---|---|
| GATE-1 | implemented | contract | `contract-write-lane` "GATE-1", against `contract-fixture` at tree `542f57f5`. An `ip_write` session the double computes as read-only registers nothing even after the double's allow-list admits it (capability drift), and the same key, allow-listed, registers. Mutation: decide by key type in `canWrite` and drop the capability check in `shouldQueueForWrite`; the drift case goes red. |
| GATE-2 | implemented | contract | `contract-write-lane` "GATE-2": a miss recorded before authorization is held while the decision is unknown, then registered once the double's answer arrives, read back from its state. Mutation: treat an unknown decision as false in `shouldQueueForWrite`; 1 test goes red. |
...
| GATE-5 | implemented | contract | `contract-write-lane` "GATE-5 and REG-8": the double refuses the first send with a seeded 500; the phrase is absent from its state, stays queued, and is held by the double after the backoff. Mutation: drop the batch on a refused send in `updateTokens`; 1 test goes red. |
| GATE-6 | implemented | contract | A writer does not report: `contract-hint-lane` "GATE-6" — a session that learned it may write sends no hint even once the double would store one from it (capability drift), with the control that a session learning it is read-only in that same state reports. Mutation: `flushHint` reports unless capability is unknown; 1 test goes red. A non-writer never attempts to register: proven at the transport seam in `never-attempt`, on the `t()` path and the content-block path, each with a control; mutations send from a read-only session on each path (1 red each). |
| GATE-7 | implemented | contract | `contract-hint-lane` "GATE-7", both directions: read-only, an unknown block's page is stored as a hint and the block is not registered; write-enabled, the block is registered. The write direction's no-report half is GATE-6's drift case. Mutation: remove `recordMissForDiscovery` from `registerContentBlock`'s read-only path; 1 test goes red. |
| GATE-8 | implemented | contract | `contract-write-lane` "GATE-8": against the double's legacy mode, which omits `write_enabled`, a write key registers and a read key does not. The write-key half is what distinguishes; the double refuses the read key's write regardless. Mutation: the legacy fallback returns false; 1 test goes red. |
| GATE-9 | implemented | contract | `contract-write-lane` and `contract-hint-lane` "GATE-9": with the project setting on, a miss in a non-base locale is neither registered nor reported, although the double would accept both; at the base locale it registers; off, it registers and reports. A miss in the gap before the first catalog, delayed by a seeded fault, is already gated by the authorize response. `base-locale-gate` pins the handshake parsing and the default, asserted on a fresh module. Mutations: no gate on the miss path (2 red); the authorize body not read (1 red); neither handshake read (2 red). The gate holds on the content-block path too: `navigation` "GATE-9 holds on the content-block path" (no registration, no report in a non-base locale; the base locale and the gate off register) and `contract-write-lane` "GATE-9 on the content-block path" against the double. Mutation: skip the gate in `registerContentBlock` (3 red). |
| GATE-10 | implemented | n/a (pure) | `resolved-marker`, 17 tests: both spellings, a bare attribute, the `false`/`0` opt-out, nearest-ancestor inheritance, both lanes suppressed on the `<Translate>` single-token path, content-block registration and `<Phrase>`, and identity untouched. The negative control: a bare `t()` under a resolved document root records its miss on the write lane and reports the page on the read lane, including beside a resolved host that registers nothing. `t()` has no subtree and is governed by GATE-9; mutating `t()` to honour the root marker reds all three. Mutations, each red: every reader path ignoring the marker, no inheritance, one spelling only, no opt-out. The JSON sibling-manifest clause is producer-side: this core takes no JSON prop hand-off. |
...
| REG-1 | implemented | n/a (pure) | `never-attempt`, at the transport seam (the API client's registration call): a session the server judged read-only sends nothing on the `t()` path or the content-block path, over ten minutes, each with a control that the same session sends once it may write. Accepted state cannot tell an unsent request from a refused one, so this is proven in-process. Mutations: a read-only session queues and sends on the `t()` path; `registerContentBlock` sends whatever the capability (1 red each). |
...
| REG-3 | implemented | n/a (pure) | In a browser the core flushes on `visibilitychange → hidden` and `pagehide`: `write-lane` "sends what is still queued when the page goes away, with keepalive". A host with no `document`, React Native, flushes through an injected signal, `setTeardownSignal`: `teardown-signal` shows the signal flushes what is queued with keepalive for a session set up before or after it, that nothing is sent without it, that a replaced or `null` signal is released first so one departure flushes once, and that a destroyed session is not flushed. Mutations, each red: register only where a document exists (3), keep a replaced signal (2), flush without keepalive (5), keep a destroyed session registered (2). |
...
| REG-8 | implemented | contract | `contract-write-lane` "GATE-5 and REG-8": a send the double refuses stays queued and lands after the backoff; the clock's 3s doubling to 5 minutes and its reset are asserted in `write-lane`. The clock lives on the `Translations` instance: a page session in a browser, the process-long object on a server, one project per instance. Mutation: drop the batch on a refused send; 1 test goes red. |
| REG-9 | implemented | contract | `contract-write-lane` "REG-9": the double advertises a batch limit of 3 and enforces it with 422; seven misses all land, and the SDK adopted the advertised limit. Mutation: hardcode the batch size to 200; 1 test goes red. |
| REG-10 | partial | - | `contract-write-lane` "REG-10": a block registration the double refuses resolves `{status:false}`, does not throw, and leaves nothing in the double's state, with a control where the same registration succeeds and is held; mutation: return `{status:true}` on a refused registration (1 red). Missing: a SKIPPED write is still success-shaped. `registerContentBlock` returns `{status:true}` when it declines to send — a read-only session, a failed catalog fetch, the base-locale gate — which the rule forbids "including a skipped write". It is exported to bindings, so changing the result is an API decision. |
...
| REG-13 | implemented | n/a (pure) | `catalog-in-flight`: queued misses are held while a catalog fetch is in flight and flushed on its arrival, so a cold visit never deduplicates against an empty catalog; a locale switch is held the same way. Mutations: no hold (3 red), no flush on arrival (2 red), the in-flight count never released (2 red). |
...
| HINT-7 | implemented | contract | `contract-hint-lane` "HINT-7": after a 429 from the double no further report is stored, even once the double's per-minute window has reset; a report the double fails with 500 is never retried over ten minutes. In-process bounds and the per-page state are in `discovery`. Mutations: a 429 does not end reporting; a failed report is retried after a minute (1 red each). |
| HINT-8 | implemented | n/a (pure) | `discovery` "never reports during SSR". Mutation: not yet recorded (CONF-3). |
| HINT-9 | implemented | contract | `contract-hint-lane` "HINT-9", the observable pair: permitted, a read-only session's page is stored; not permitted, the session sends nothing even once the double would accept its hint (capability drift). Mutation: ignore `auto_discovery` in `flushHint`; 1 test goes red. |
...
| HINT-13 | implemented | contract | `contract-hint-lane` "HINT-13", the rule's own Test against `contract-fixture`: a persistent layout bound to `t` has its miss stored for page B after `notifyNavigation()`, and without the call nothing is stored for B although the double would store it. In the core, `navigation`: a single-token `Translate`, an unknown content block and a `Phrase` that stay mounted each record at the new URL; the controls, a `Translate` and a `Phrase` on a node removed from the page but never destroyed, record nothing; a second call for the same URL reports nothing more; a registered phrase or block is not sent again; the call itself sends nothing. Mutations, each red: no fresh `t` published (3), the navigation epoch not advanced (4), `Translate` re-entering a detached host (1), `Phrase` re-entering a detached host (1), a mounted block not re-recorded (1), a known block re-registered (1). |
| ICU-1 | implemented | n/a (pure) | Cross-implementation fixture: `interpolation-cross-impl`, all 25 rows of langsys-php's `interpolation-reference.json`, vendored @ `c11a711` blob `017bffdd1d83` and each asserted through both `interpolate` and `t()`, plus `interpolate` "missing select arguments fall back to `other`". The fixture cannot reach the call paths, since every row supplies params, and they returned the raw source until the JS Server lane executed one. `icu-no-params` shows that with no params `t()` in both overloads, a catalog hit, and `<Translate>` (a single token, a multi-token block at the base locale, an attribute) render `other`; that supplied params still choose, including on the single-token path that used to interpolate `t()`'s result a second time; and that plain text with braces is left as written. Mutations, in an isolated copy: `t()` returns early without params (4 red); `applyParams` returns early (2 red); the base-locale walk skips ICU blocks again (2 red); the single-token path applies params to `t()`'s result (1 red); revert `translations.ts` and `translate.ts` (7 red). The fixture's no-params rows omit the argument where a row omits `params`: requiring `interpolate`'s params again reds those two rows on `interpolate`, and restoring `t()`'s early return reds the same two on `t()`. |
| ICU-2 | implemented | n/a (pure) | Cross-implementation fixture: `interpolation-cross-impl`, all 25 rows of langsys-php `interpolation-reference.json`. same fixture; plus `interpolate` "treats null and undefined as absent" Mutation: not yet recorded (CONF-3). |
| ICU-3 | implemented | n/a (pure) | Cross-implementation fixture: `interpolation-cross-impl`, all 25 rows of langsys-php `interpolation-reference.json`, plus the nested plural/select cases where `#` with no count renders `{argName}`. On the call paths, `icu-no-params` shows `t()` rendering `{count} items` for a plural given no count, and a `<Translate>` block at the base locale doing the same. Mutations: `t()` returning early without params reds the `t()` case; `applyParams` returning early reds the block case. |
| ICU-4 | implemented | n/a (pure) | `interpolate` "debug notice for defaulted arguments" — names the argument and locale, deduped per template+locale, silent unless `logger.debugEnabled`, fires for plural as well as select. Both directions asserted. A host with its own logger receives the notice instead, through `interpolate`'s `onDefaulted` option, or computes the names with `defaultedArguments`, which walks the same recovery: `interpolate` "notices routed to the caller" asserts the handler gets every defaulted name once, the template and the locale on every call, the core logger then stays silent even under debug (control: without a handler it fires), nothing is reported when every argument is supplied, and `defaultedArguments` equals what the handler reported across seven templates, `null` counting as missing. NOT corroborated: a debug-only emission has no counterpart in the shared fixture, which asserts rendered output. Mutations, in an isolated copy: the handler ignored (3 red); the core notice fired as well (1); names not unique (2); `defaultedArguments` empty (1). |
| ICU-5 | implemented | n/a (pure) | Cross-implementation fixture. `interpolation-cross-impl` (25 rows) plus `interpolate` "a recovered argument survives the format call" — five vectors, mutation-checked against restoring the argument node, which reproduces the PHP lane's live symptom. Conforming by remove-binding-sites, which the amended clause names explicitly |
| ICU-6 | implemented | n/a (pure) | Cross-implementation fixture: the two ICU-6 rows of langsys-php's `interpolation-reference.json` (count 3 and count 1), asserted through `interpolate` and `t()` in `interpolation-cross-impl`; on PHP's formatter they exercise its own branch selection, here they pass natively. `icu-formatter-failure` (16 tests). The spec's vector `You have {count, plural, one {{count} car} other {{count} cars}}` at `en` renders `You have 3 cars` and `You have 1 car` natively through `interpolate` and `t()`, with no warning. Because `intl-messageformat` renders it, the fallback is proved with a forced formatter failure (the module wrapped so `format()` throws after a successful parse): the same vector renders the same through both paths; an exact `=N` wins, then the CLDR category in the render locale (`ru` few/many/one), then `other`; `#` renders the value after the offset; select, nested constructs and CLDR number formatting; an unsupplied value stays `{argName}`; never `""`, never the raw construct. A host with its own logger receives the failure through `interpolate`'s `onFormatterFailure` option instead, on every call, with the core logger silent; mutations ignoring the handler (1 red) or firing the core warning as well (1) are red. The warning fires with debug logging off, names the phrase, the locale and the formatter's error, and is deduplicated per (template, locale); a recovered missing argument does not fire it. A phrase that does not parse has no branches to select and takes the simple path. Mutations, in an isolated copy: simple path instead of own branch selection (11 red); warning gated on debug (2); no dedupe (1); no exact `=N` (2); no CLDR category (4); offset ignored (1); no warning (2); warning on a recovered missing argument (6); `#` left literal (5). |
...
| TOK-2 | implemented | n/a (pure) | The collapse set is asserted on the canonicalization function in `tokenizer-convergence` (23 members, 4 non-members, built from code points, with the Phrase path). The C0 strip is asserted in `c0-strip` (47 tests): each of the 28 characters U+0001–U+0008, U+000B, U+000C and U+000E–U+001F removed, not mapped to a space; TAB, LF and CR collapse; NUL, U+007F and the C1 range kept; strip before collapse before trim; on every path — text node, attribute value, `<Phrase>` key, `t()` key on lookup and on registration, and `lookup()`. `t()` still renders the phrase it was given. Cross-lane: six rows in `canonicalization-reference.json` (`along-reference`, `fs-in-text`, `vt-in-text`, `tab-in-text`, `del-in-text`, `fs-in-attr`); `langsys-php-sdk` agrees on all six at `3121404`, which carries its own C0 strip; `fs-in-attr` is the row that proves the strip rather than the parser, since libxml2 keeps C0 in attribute values. Mutations, in an isolated copy: no strip (34 red); strip mapped to a space (35); collapse before strip (4); `t()` key not stripped (2); `lookup()` not stripped (1); strip set narrowed (4). |
...
| TOK-6 | partial | - | `translate` "an attribute-only token must not take the single-token fast path" and "still uses the fast path for a genuine single TEXT token"; a unit whose one text node sits beside a nested block host registers as a phrase (`nested-host-excision`); attributes precede content in `tokenizer-cross-impl`. Missing: the search for a unit's single text node still counts text inside an excluded element or a nested phrase host, so `Hello <script>…</script>`, whose one token is `Hello`, registers as a content block instead of a phrase (measured). |
...
| MARK-2 | partial | - | Both spellings of a phrase host are read, on the block walk and on the `<Translate>` walk, in `content-block-identity`. Missing: a phrase-marked host that no `<Phrase>` manages is skipped by both walks, so it never registers; the rule requires it to register whole, as the one string its host defines, on a miss. |
| MARK-3 | partial | - | The value reading holds on every walk: bare, empty, `true` or an id marks a host; `false` and `0` opt out, trimmed and case-insensitive (`nested-host-excision`). Missing: an identity value is not adopted as the host's `custom_id` (the core re-derives it), and a bare declaration met by a walk is excised but not registered as a block of its own. `1` and `yes`, declarations from 8.2.8, mark a host as every non-opt-out value does; which of them declares and which identifies matters only to that missing half. |
| MARK-4 | implemented | n/a (pure) | `nested-host-excision`, the rule's own Test: a unit holding a nested stamped block host and a nested phrase host keeps neither's text, and each nested host registers once, on its own; the control sets the nested block marker to `false` and its text folds back in. Also asserted: both spellings, a bare or `true` declaration excised, `0` and ` FALSE ` not; the enclosing render never writes into a nested block, even on re-render; the enclosing block's id no longer depends on the nested content. A `Translate` stamps its host while it is constructed, so a nested host is excised when it is constructed before the enclosing one, which is the order component frameworks mount in. Mutations, each red: no excision in the tokenizer walk (9), the opt-out ignored (3), one spelling only (3), the render walk writing into a nested block (1), the single-text-node search counting a nested block (1). |
...
| SRV-6 | n/a (profile: server) | - | Profile server, and a binding for a render inside a server request scope; this is the browser core. |
...
| CACHE-2 | implemented | contract | `contract-write-lane` "CACHE-2": after the double fails one catalog fetch, a lookup inside the window renders source text although the double would now answer, and after the window the translation renders. `catalog-failure-window` proves at the transport seam that concurrent fetches for a pair share one request, the window runs 3s, 6s, 12s to at most 5 minutes, the first success clears it, and an explicit refresh still fetches. Mutations, each red: no window, no request sharing, no doubling, no reset on success, a refresh held by the window. |
| OBS-1 | implemented | contract | `contract-write-lane` "OBS-1": an `ip_write` key the double computes as unable to write produces exactly one notice across several misses; a read key produces none, including right after a refused `ip_write` session, because authorization passes the key type it received rather than the previous init's. Mutation: remove the notice latch; 1 test goes red. |
...
| WIRE-2 | implemented | contract | `contract-hint-lane` "WIRE-2": the double's hint route answers 204 with no body and no content type; the SDK treats it as sent, the hint is stored, and nothing is logged as a failure on either log channel. Mutation: parse every body as JSON, 204 included; 1 test goes red. |
| WIRE-3 | implemented | contract | Lowercase `xx-yy` internally and on the wire: `locale` WIRE-3 block and the `api` wire assertion. The read key for no-category entries: `contract-write-lane` "WIRE-3" registers a block with no category and the next session finds it known under `__uncategorized__`, as the double serves it. Mutation: look up an uncategorised block under `''`; 1 test goes red. |
| WIRE-4 | implemented | contract | `contract-write-lane` "WIRE-4": with the double dropping the catalog connection, `t()` returns source text and does not throw, and no miss reaches the double. In-process, `catalog-failure` covers a throwing client, a failed locale switch, the held queue and recovery. Two gates stop an outage registering, on the miss path and in the flush; removing both reds the contract test, and each alone is covered by the other. |
...
| CONF-1 | implemented | n/a (pure) | A meta-rule over how rows are proven. Every API-dependent row asserts on what the contract double accepted or on what the page renders, never on a spy or a payload; a "never attempt" clause is proven at the transport seam, where accepted state cannot answer it. Rows spanning a `t()` path and a content-block path name both. The evidence that follows the rule is `contract-write-lane`, `contract-hint-lane` and `never-attempt`. |
| CONF-2 | implemented | n/a (pure) | Every row records a tier; `_dev_/tally-conformance.mjs` checks the canonical format and status against tier, and `conformance-structure` runs it against planted defects that must each fail. The fleet's contract double is authored here, `contract-fixture` at tree `542f57f5`, with 34 self-tests in `contract-fixture`, each "says no" case paired with the condition that permits it; ten mutants of the double each go red. |
| CONF-3 | partial | - | Every mutation runs in an isolated copy of the working tree, never in `src/`, because the bindings and JS Server load this repo through a symlink. SSR strategy cases run one process each (`ssr-strategy-isolation`, with a control that reds). A re-appliable mutation is recorded for: HINT-5, HINT-7, HINT-9, HINT-10, HINT-12, HINT-13, ICU-1, ICU-3, ICU-5, ICU-4, ICU-6, TOK-2, MSG-1, MSG-2, MSG-5, MSG-6, MIG-1, MIG-2, MIG-3, MIG-4, MIG-5, MIG-6, MIG-7, SNAP-2, SNAP-3, CID-1, CID-3, CID-4, TOK-1, TOK-4, TOK-5, MARK-1, SRV-4, GRANT-3, CACHE-1, CACHE-2, OBS-1, CAT-3, MARK-4, GATE-1, GATE-2, GATE-3, GATE-4, GATE-5, GATE-6, GATE-7, GATE-8, GATE-9, GATE-10, REG-1, REG-3, REG-8, REG-9, REG-10, REG-12, REG-13, SSR-2, WIRE-2, WIRE-3, WIRE-4. Missing a recorded mutation: CAT-1, CAT-2, REG-2, REG-4, REG-5, REG-6, REG-7, REG-11, HINT-1, HINT-3, HINT-4, HINT-6, HINT-8, HINT-11, ICU-2, CID-2, TOK-3, TOK-6, SSR-1, GRANT-1, GRANT-2, GRANT-4, WIRE-1, WIRE-5 (GRANT-1 for its runtime half). |
| MSG-1 | implemented | n/a (pure) | `resolveServerMessages(body, { key, resolver, pieces })` reads entries only where configuration says: the dotted `key` the server attaches them under, or an app `resolver`; with neither it throws. The framework's body is never searched by shape and never changed. An entry needs a `template` or a `message`, and `pieces` maps other piece names. The vector file `server-message-vectors.json` is authored here for the fleet from real framework messages: 12 canonical entries (Laravel 10.50.2, Pydantic 2.13.5, Django 4.2.30, DRF 3.16.1, ActiveModel 8.1.3.1, each citing its source sentence), 13 marker, 10 fill, 10 resolve and 12 render rows, asserted by `server-messages` (73 tests). Its resolve rows: Laravel's and FastAPI's native 422 bodies under `langsys_errors`, a DRF body with a nested key holding a field map, a Rails-shaped body with renamed pieces (each body unchanged), a key absent, entry-shaped objects elsewhere in the body not resolved, a message-only entry, an object with neither piece rejected, no `code` where the framework has none, a single entry at the key. The same entries render identically from two frameworks' bodies. Mutations, in an isolated copy: `template` required (1 red); `pieces` ignored (2); no throw without configuration (1); a field map not flattened (1); falling back to the whole body (2); the resolver mutating the body (4). |
| MSG-2 | implemented | n/a (pure) | `code` and `field` are the framework's own, passed through unchanged: Pydantic's `string_too_short` and its `loc` array arrive as sent, and an entry whose framework has no code carries none. No vocabulary is exported (asserted). `renderServerMessage` never reads `code`: render row `code-does-not-choose-text` renders Laravel's shared `in`/`exists` sentence the same under either code. The vector file `server-message-vectors.json` is authored here for the fleet from real framework messages: 12 canonical entries (Laravel 10.50.2, Pydantic 2.13.5, Django 4.2.30, DRF 3.16.1, ActiveModel 8.1.3.1, each citing its source sentence), 13 marker, 10 fill, 10 resolve and 12 render rows, asserted by `server-messages` (73 tests). Mutations, in an isolated copy: an array `field` dropped (2 red); a `code` invented where none was sent (3). |
| MSG-5 | implemented | n/a (pure) | `renderServerMessage` renders `t(template, category, params ?? {})` when the catalog holds the template, the entry's `message` otherwise, and the template filled when there is no `message`. An entry with no `template` shows its `message` and reaches no lookup path (asserted on `lookup`, `t` and `tSignal`). The vector file `server-message-vectors.json` is authored here for the fleet from real framework messages: 12 canonical entries (Laravel 10.50.2, Pydantic 2.13.5, Django 4.2.30, DRF 3.16.1, ActiveModel 8.1.3.1, each citing its source sentence), 13 marker, 10 fill, 10 resolve and 12 render rows, asserted by `server-messages` (73 tests). Its render rows: no catalog, template not translated, and a server-localised `message` kept on a miss; Laravel translations with and without params; a Pydantic translation; an ICU plural translation from ActiveModel's count param at 1 and 12; `message` present as a catalog key and still never looked up; a message-only entry. Mutations, in an isolated copy: `message` as the key (4 red); a message-only entry looked up (1); a miss filling the template over `message` (1). |
| MSG-6 | implemented | n/a (pure) | One category, `Errors` by default, set with `messagesCategory` in `init`. Render row `other-category-misses`: a template held under `Errors` misses under `Validation` and falls back. `server-messages` also shows the unset default renders under `Errors`, and a configured `Validation` renders there while `Errors` then misses. The vector file `server-message-vectors.json` is authored here for the fleet from real framework messages: 12 canonical entries (Laravel 10.50.2, Pydantic 2.13.5, Django 4.2.30, DRF 3.16.1, ActiveModel 8.1.3.1, each citing its source sentence), 13 marker, 10 fill, 10 resolve and 12 render rows, asserted by `server-messages` (73 tests). Mutations, in an isolated copy: default category changed (2 red); configured category ignored (1). |
| MSG-3 | n/a (profile: server) | - | Profile server; this is the browser core. |
| MSG-4 | n/a (profile: server) | - | Profile server; this is the browser core. |
| MSG-7 | n/a (profile: server) | - | Profile server; this is the browser core. |
| MSG-8 | n/a (profile: server) | - | Profile server; this is the browser core. |
| MSG-9 | n/a (profile: server) | - | Profile server; this is the browser core. |
| MSG-10 | n/a (profile: server) | - | Profile server; this is the browser core. |
| MSG-11 | n/a (profile: server) | - | Profile server; this is the browser core. |
| MSG-12 | n/a (profile: server, binding) | - | Profiles server and binding; this is the core. |
| MIG-1 | implemented | n/a (pure) | The mode is `legacyKeys` in `init`, and `t()` consults a resolver only when one is set. `legacy-keys` asserts the unset default on an instance nothing configured (a key-shaped argument is literal source text), and the same call resolving once the mode is set. Mutation, in an isolated copy: the resolver never consulted (9 red). |
| MIG-2 | implemented | n/a (pure) | A hit is the key's value; a miss is the argument as literal text, which Langsys `t()` does not convert. `convertLegacyCall` converts a literal miss by entry point: `t` nothing, the `vue-i18n` and `i18next` bridges by their format's row. The vector file `mig-vectors.json` is authored here for the fleet (70 rows: 27 value conversions, 6 plural-form sets, 13 entry-point calls, 18 resolutions, 6 refusals, each naming its format), asserted by `legacy-keys` (76 tests) for every row in this core's formats. `langsys-php-sdk`, run read-only, agrees on every `laravel` and entry-point row and on the `plain` rows it implements; it does not yet convert 8.2.11's Rails and Python placeholder spellings (8 `plain` rows). Its call rows for this core's entry points pass, and every `same_phrase_as` pair registers one phrase. Mutations, in an isolated copy: the key registered instead of its value (8 red); a flat key's category dropped (1). |
| MIG-3 | implemented | n/a (pure) | `legacy-keys`: On a hit the phrase is the resolved value, never the key: registering `checkout.greet` queues the same category and phrase as calling `t('Hello {name}', 'checkout')`, and resolution row `key-string-never-the-phrase` resolves the value. Mutation, in an isolated copy: the key string registered (8 red). |
| MIG-4 | implemented | n/a (pure) | `convertLegacyValue` and `convertLegacyPluralForms`. The vector file `mig-vectors.json` is authored here for the fleet (70 rows: 27 value conversions, 6 plural-form sets, 13 entry-point calls, 18 resolutions, 6 refusals, each naming its format), asserted by `legacy-keys` (76 tests) for every row in this core's formats. `langsys-php-sdk`, run read-only, agrees on every `laravel` and entry-point row and on the `plain` rows it implements; it does not yet convert 8.2.11's Rails and Python placeholder spellings (8 `plain` rows). Placeholders convert the same way in every format (`{{name}}`, `:name`, `%{name}`, `%(name)s`, `%(name)d`; `%%` literal); formatted and positional placeholders and `:Name` stay verbatim with an issue. The same pipe string is a plural under `vue-i18n` and verbatim under `plain`; `{count}` never appears inside a converted branch; branches are canonical. In `t()`, an unrecognised value registers as written and warns at every level, once, naming file and key. Mutations, in an isolated copy: `plain` pipes converted (5 red); vue two forms as `one` (5); `{{count}}` not `#` (9); `%%` kept (2); `:Name` accepted (1); positional `%s` accepted (1); the value warning debug-only (1); no dedupe (1). |
| MIG-5 | implemented | n/a (pure) | `legacy-keys`: A path-shaped key's first segment is the category (`checkout.submit` → `checkout`, nested or flat), a `namespace` file's namespace is its keys' category, and an explicit category wins, both in resolution rows and through `t()`'s queue. A sentence key has none. Mutations, in an isolated copy: the resolved category beating the call's (1 red); the namespace ignored (2). |
| MIG-6 | implemented | n/a (pure) | `legacy-keys`: A key absent from the files registers its argument as literal text and warns at debug, naming it; with debug off it is silent. A changed value registers as a new phrase. Mutation, in an isolated copy: the miss warning at every level (1 red). |
| MIG-7 | implemented | n/a (pure) | `legacy-keys`: `createLegacyKeys([{ name, format?, namespace?, data }])`: this core reads `i18next`, `vue-i18n` and `plain` (the default); every other format, and a `.php`, `.yml`, `.po` or `.mo` name, throws `LegacyFormatError` naming the format and the file, and a `.mo` names its `.po`. `init` throws the same, asserted. Nested keys resolve by path, the first configured file answers, and `duplicates()` reports the rest. The vector file's six refusal rows each refuse here; a control shows each supported format is not refused. Mutations, in an isolated copy: no refusal (6 red); the `.mo` hint dropped (1); the last file winning (1); `vue-i18n` the default (3); i18next pairing off (3). The PHP-array and YAML clauses belong to the cores that read those formats. |
| MIG-8 | implemented | n/a (pure) | `legacy-keys`: The JS entry points are the browser `t()` mode and a server `t()`; both convert through the resolver and converters exported identically from `/pure` and the main entry (asserted as the same objects), and the browser mode queues exactly the phrase `createLegacyKeys` resolves for the same configuration. Laravel's `__()` is PHP's entry point and reads formats this core refuses (MIG-7), so its agreement with `t()` is carried by the vector file's paired rows (`same_phrase_as`), which PHP runs. |
| MIG-9 | n/a (profile: server) | - | Profile server; this is the browser core. |
| SNAP-1 | n/a (profile: server) | - | Profile server; this is the browser core. |
| SNAP-2 | implemented | n/a (pure) | `snapshot`: `LangsysApp.loadSnapshot` parses and verifies a `langsys-catalog-snapshot` and publishes the locale's catalog synchronously, `t()` returning its translation on the next line with no fetch; a locale the snapshot lacks loads nothing; the snapshot passed in is not mutated. Registration is decided only against the live catalog (REG-13): while the published catalog is a snapshot, a phrase it holds is still queued (for registration, not for discovery, so a read-only session reports nothing for it), and the queue is held rather than deduplicated against the snapshot; once the live catalog arrives, what it holds is dropped and what it lacks is registered. Mutations, in an isolated copy: no tentative queue (1 red); no hold (1); the snapshot flag never set (1) or never cleared (2); a tentative phrase reported for discovery (1). Once `change()` runs the catalog is fetched, replaces the snapshot and supplies a phrase it lacked; with the fetch failing, the snapshot keeps rendering and the missing phrase is source text. Every row of `snapshot-vectors.json` (authored here: 8 rows, 4 refusals, 1 any-encoding load, expected bytes and digests from an independent encoder) serialises byte for byte, checksums, and loads; `sha256Hex` agrees with `node:crypto` across block boundaries and non-ASCII. Mutations, in an isolated copy: default `sort()` (2 red); `Object.keys` order (23); an empty map as `[]` (2); U+2028 escaped (2); `format` hashed (23); `base_locale` unhashed (24); the SHA length word wrong (24); the snapshot not cloned (4). |
| SNAP-3 | implemented | n/a (pure) | `snapshot`: a snapshot is never the catalog of record. Loading one does not mark the locale fetched, so the catalog fetch runs and replaces it, while `seedCatalog`, the SSR hand-off, does suppress the fetch (the control). An edited snapshot is refused by its checksum, and the loader refuses a different format, an unsupported version and a missing member, each by name; the README gives re-export as the refresh path. Mutations, in an isolated copy: the snapshot marked fetched (1 red); no checksum check (2); no version check (1); no member check (1); no format check (1). |
...
1. **Rules not yet implemented, or partly.** MARK-2's whole-host registration; MARK-3's adoption of a stamped id and registration of a bare declaration; TOK-6's single-text-node search, which still counts text inside an excluded element or a nested phrase host; REG-10's skipped write, which `registerContentBlock` still reports as `{status:true}`. Each is graded in its row.

2. **HINT-6 — resolved, retained for the record.** The SDK preserved hashbang fragments while
...
5. **REG-11 — the permitted suppression half is not implemented.** The warning is in and nothing is skipped, which conforms. The optional second signal — suppress when a longer catalog entry shares the prefix — would need a prefix scan of the catalog on every miss, and buys only the pollution case the warning already surfaces. Recorded as a deliberate omission rather than a gap.

6. **OBS-1's notice covers capability, not every inert state.** The write-capability warning is now above debug on both channels. HINT-9's reporting-disabled case stays debug-only deliberately — the customer chose that setting, so it is a configuration, not a fault.

## Not applicable

`HINT-2`, `SRV-6`, `MSG-3`, `MSG-4`, `MSG-7` to `MSG-11`, `MIG-9` and `SNAP-1` (`n/a (profile: server)`), `SRV-1`, `SRV-2`, `SRV-3`, `SRV-5` and `MSG-12` (`n/a (profile: server, binding)`) and `BIND-1` to `BIND-6` (`n/a (profile: binding)`).