...
| **specVersion** | 8.2.7 |
| **Spec revision read** | langsys2 54c79ab4…, docs/sdk-spec.mdx blob 5fa32cb980b91bec41f40677638f47139a0fbfbb (specVersion 8.2.7). Re-derived with `git -C ~/Documents/dev/langsys2 ls-tree 54c79ab4 docs/sdk-spec.mdx` at this write and checked by `npm run verify:spec`, which also fails if the vector file cites a different blob. Every rule id is counted by `npm run tally:conformance`. |
| **SDK revision** | `feature/838_write_key_gating_reland`, cut from `origin/main` `2d7b11f` (v0.6.5) |
| **Suite** | 1015 tests in 57 files, `npm test`, counted at the tip of this branch |
...
| MARK-3 | partial | - | The value reading holds on every walk: bare, empty, `true` or an id marks a host; `false` and `0` opt out, trimmed and case-insensitive (`nested-host-excision`). Missing: an identity value is not adopted as the host's `custom_id` (the core re-derives it), and a bare declaration met by a walk is excised but not registered as a block of its own. |
...
| CONF-3 | partial | - | Every mutation runs in an isolated copy of the working tree, never in `src/`, because the bindings and JS Server load this repo through a symlink. SSR strategy cases run one process each (`ssr-strategy-isolation`, with a control that reds). A re-appliable mutation is recorded for: HINT-5, HINT-7, HINT-9, HINT-10, HINT-12, HINT-13, ICU-1, ICU-3, ICU-5, ICU-6, TOK-2, CID-1, CID-3, CID-4, TOK-1, TOK-4, TOK-5, MARK-1, SRV-4, GRANT-3, CACHE-1, CACHE-2, OBS-1, CAT-3, MARK-4, GATE-1, GATE-2, GATE-3, GATE-4, GATE-5, GATE-6, GATE-7, GATE-8, GATE-9, GATE-10, REG-1, REG-3, REG-8, REG-9, REG-10, REG-12, REG-13, SSR-2, WIRE-2, WIRE-3, WIRE-4. Missing a recorded mutation: CAT-1, CAT-2, REG-2, REG-4, REG-5, REG-6, REG-7, REG-11, HINT-1, HINT-3, HINT-4, HINT-6, HINT-8, HINT-11, ICU-2, ICU-4, CID-2, TOK-3, TOK-6, SSR-1, GRANT-1, GRANT-2, GRANT-4, WIRE-1, WIRE-5 (GRANT-1 for its runtime half). |
| MSG-1 | not implemented | - | The core has no server-message entry handling; entries are neither resolved nor rendered, and `server-message-vectors.json` is not yet authored. |
| MSG-2 | not implemented | - | The core has no server-message entry handling; entries are neither resolved nor rendered, and `server-message-vectors.json` is not yet authored. |
| MSG-5 | not implemented | - | The core has no server-message entry handling; entries are neither resolved nor rendered, and `server-message-vectors.json` is not yet authored. |
| MSG-6 | not implemented | - | The core has no server-message entry handling; entries are neither resolved nor rendered, and `server-message-vectors.json` is not yet authored. |
...
1. **Rules not yet implemented, or partly.** MARK-2's whole-host registration; MARK-3's adoption of a stamped id and registration of a bare declaration; TOK-6's single-text-node search, which still counts text inside an excluded element or a nested phrase host; REG-10's skipped write, which `registerContentBlock` still reports as `{status:true}`; MIG-1 to MIG-8, SNAP-2 and SNAP-3; MSG-1, MSG-2, MSG-5 and MSG-6. Each is graded in its row.
