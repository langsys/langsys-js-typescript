...
| **Suite** | 526 tests in 30 files, `npm test`, counted at the tip of this branch |
...
now records **25 rows, 25 agree, 0 diverge** where it recorded 13 of 19 (23 after the four specified rows, then `math-subtree` and `svg-inline-icon` with the TOK-1 fix). The stale half of that is
