...
| **specVersion** | 8.2.6 |
| **Spec revision read** | langsys2 0871ed72…, docs/sdk-spec.mdx blob 9e826c918fe98ef2df6f47e7dd2c287f3de81232 (specVersion 8.2.6). Re-derived with `git -C ~/Documents/dev/langsys2 ls-tree 0871ed72 docs/sdk-spec.mdx` at this write and checked by `npm run verify:spec`, which also fails if the vector file cites a different blob. Every rule id is counted by `npm run tally:conformance`. |
| **SDK revision** | `feature/838_write_key_gating_reland`, cut from `origin/main` `2d7b11f` (v0.6.5) |
| **Suite** | 940 tests in 55 files, `npm test`, counted at the tip of this branch |
...
| TOK-2 | partial | - | The collapse set is asserted on the canonicalization function in `tokenizer-convergence` (23 members, 4 non-members, built from code points, with the Phrase path). Missing: the C0 strip — U+0001–U+0008, U+000B, U+000C and U+000E–U+001F removed before collapse on every register and lookup path, with VT and FF removed rather than collapsed. `normalizeTokenText` still collapses VT and FF to a space. |
...
1. **Rules not yet implemented, or partly.** TOK-2's C0 strip; MARK-2's whole-host registration; MARK-3's adoption of a stamped id and registration of a bare declaration; TOK-6's single-text-node search, which still counts text inside an excluded element or a nested phrase host; REG-10's skipped write, which `registerContentBlock` still reports as `{status:true}`; MIG-1 to MIG-8, SNAP-2 and SNAP-3; MSG-1, MSG-2, MSG-5 and MSG-6. Each is graded in its row.
