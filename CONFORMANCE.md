...
| **Suite** | 915 tests in 53 files, `npm test`, counted at the tip of this branch |
...
| REG-10 | implemented | contract | `contract-write-lane` "REG-10": a block registration the double refuses resolves `{status:false}`, does not throw, and leaves nothing in the double's state, with a control where the same registration succeeds and is held. Mutation: return `{status:true}` on a refused registration; 1 test goes red. |
...
| HINT-13 | not implemented | - | No `notifyNavigation()`. A persistent layout that stays mounted across a route change records nothing for the new URL. |
...
1. **Rules not yet implemented, or partly.** TOK-2's C0 strip; MARK-2's whole-host registration; MARK-3's adoption of a stamped id and registration of a bare declaration; TOK-6's single-text-node search, which still counts text inside an excluded element or a nested phrase host; HINT-13; MIG-1 to MIG-8, SNAP-2 and SNAP-3; MSG-1, MSG-2, MSG-5 and MSG-6. Each is graded in its row.
