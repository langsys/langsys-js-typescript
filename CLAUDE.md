...
`npm test` runs vitest over `tests/` — 29 files, 424 tests (`api-reachability`, `api`, `cache-scope`, `catalog-envelope`, `content-block-identity`, `custom-id-cross-impl`, `custom-id`, `discovery`, `ellipsis-warning`, `grant-lane`, `init-settle`, `interpolate`, `interpolation-cross-impl`, `langsys-app`, `locale`, `logger`, `no-private-fields`, `obs-notice`, `persist`, `pure-subpath`, `richtext`, `seed-catalog`, `tfunction-identity`, `tokenizer-convergence`, `tokenizer-cross-impl`, `translate`, `translations`, `write-lane`).
