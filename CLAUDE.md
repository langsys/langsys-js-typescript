...
`npm test` runs vitest over `tests/` — 25 files, 359 tests (`api-reachability`, `api`, `cache-scope`, `catalog-envelope`, `content-block-identity`, `custom-id-cross-impl`, `custom-id`, `discovery`, `ellipsis-warning`, `grant-lane`, `init-settle`, `interpolate`, `interpolation-cross-impl`, `langsys-app`, `locale`, `logger`, `no-private-fields`, `obs-notice`, `persist`, `richtext`, `tfunction-identity`, `tokenizer-cross-impl`, `translate`, `translations`, `write-lane`).
