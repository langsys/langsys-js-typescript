...
src/
    ...
    legacy-value.ts         # Legacy i18n values to Langsys/ICU text (MIG-4): placeholders, plurals by format, literal-miss conversion by entry point
    legacy-keys.ts          # createLegacyKeys: resolve legacy keys against parsed source files, refuse other ecosystems' formats (MIG-2/5/7)
    snapshot.ts             # Catalog snapshots (SNAP): canonical serialisation, sync sha256, parse/refuse, build; loading is LangsysApp.loadSnapshot
    server-messages.ts      # Server message entries (MSG): resolveServerMessages, templateMarkers, fillTemplate; rendering is LangsysApp.renderServerMessage
...
contract-fixture/           # The fleet's API contract double (spec CONF-2) — not shipped in the package
    server.mjs              # Runnable HTTP double, Node built-ins only
    seed.schema.json        # The seed document for POST /__fixture/seed
    README.md               # What it enforces, in the backend's order, and what it does not model
```
...
notifyNavigation()                      // Route changed: re-look-up mounted content for the new URL
...
- `npm run verify:spec` — checks that CONFORMANCE.md's **Spec revision read** row names the blob the commit it cites actually carries, by `git ls-tree` against a sibling `langsys2` checkout, and that `tests/fixtures/canonicalization-reference.json` cites the same blob in `spec_blob`. It reads the canonical header (`langsys2 <commit>…, docs/sdk-spec.mdx blob <40-hex>`). Run it **before** writing that row: spec 8.0.1 requires the revision to be re-derived on every write rather than carried. Not in CI and not a vitest case, because it needs the sibling repo and could only skip there. Exits 2 rather than 0 when it cannot verify.
- `npm run tally:conformance` — checks CONFORMANCE.md against the canonical conformance format and the spec blob it cites: one status table, one rule id per row, all 79 ids, a canonical status and tier on each, status checked against tier, and a named test that exists for every implemented or provisional row. Exits 0 green, 1 structural error, 2 cannot check, 3 valid but not green. `--structure-only` needs no spec, and `conformance-structure` runs it that way in CI with planted defects that must each fail.
...
4a. **No ECMAScript `#private` fields, anywhere in the core.** Use TypeScript's `private` — it is erased at build time and carries no runtime restriction. `#private` is keyed to the real instance, so reading one through a `Proxy` throws `TypeError: Cannot read private member`, and Vue and Solid forward core methods through a Proxy **unbound** to preserve identity: `proxy.method()` runs with `this` set to the proxy. One `#private` field therefore breaks those bindings at runtime, in whichever method touches it, with nothing in the types or the build to warn you. Pinned by `tests/no-private-fields.test.ts`, which scans `src/**/*.{ts,mts,cts,tsx}` and the package build, and carries its own positive control — a blinded scanner fails that control rather than silently reporting "none found". The dist half matches esbuild's **lowering helpers** (`__privateAdd`/`__privateGet`/…) as well as a literal `#`, because at our `es2021` target a private field is downlevelled over a WeakMap: `dist` contains no `#` at all, while the lowered form still throws through a Proxy. A missing `dist` fails the check rather than skipping it, so **`npm run build` must precede `npm test`** — `ci.yml` is ordered that way deliberately.
...
`npm test` runs vitest over `tests/` — 60 files, 1216 tests (`api`, `api-reachability`, `base-locale-gate`, `c0-strip`, `cache-scope`, `canonicalization-agreement`, `catalog-block-shape`, `catalog-envelope`, `catalog-failure`, `catalog-failure-window`, `catalog-in-flight`, `changelog-completeness`, `conformance-structure`, `content-block-identity`, `contract-fixture`, `contract-hint-lane`, `contract-write-lane`, `custom-id`, `custom-id-cross-impl`, `discovery`, `ellipsis-warning`, `grant-documentation`, `grant-lane`, `icu-formatter-failure`, `icu-no-params`, `init-settle`, `interpolate`, `interpolation-cross-impl`, `langsys-app`, `legacy-id-tolerance`, `legacy-keys`, `locale`, `logger`, `marker-exports`, `navigation`, `nested-host-excision`, `never-attempt`, `no-private-fields`, `no-raw-invisibles`, `obs-notice`, `persist`, `pure-subpath`, `resolved-marker`, `rich-phrase-identity`, `richtext`, `seed-catalog`, `server-messages`, `snapshot`, `ssr-precondition-doc`, `ssr-strategy-isolation`, `structural-block-detection`, `teardown-signal`, `tfunction-identity`, `tokenizer-convergence`, `tokenizer-cross-impl`, `translate`, `translations`, `write-decision-cache`, `write-decision-persistence`, `write-lane`).
...
**Cross-SDK behaviour is verified against langsys-php's fixtures, not re-derived here.** `langsys-php/tests/fixtures/` holds the shared contract — `custom-id-reference.json` (13 cases), `tokenizer-reference.json` (17, re-vendored at blob `5689f3c1` after PHP corrected its script/style row), `interpolation-reference.json` (25, vendored at blob `017bffdd` from `c11a711`, with its no-params rows, which `interpolation-cross-impl` also runs through `t()`, and ICU-6's two rows). **All three are vendored into `tests/fixtures/` and asserted every run** — `custom-id-cross-impl`, `tokenizer-cross-impl`, `interpolation-cross-impl` (see CONFORMANCE CID-1/ICU-1..6). Re-vendor on a deliberate bump, citing the source SHA — never edit the vendored copy. Execute the *published tarball* against all three after a release; re-deriving expected values in TypeScript proves nothing, since both sides would share the same mistake. Adding cases is safe; changing an existing expectation is a breaking change to content-block identity in every SDK and goes to the PHP agent first. One shared fixture comes from langsys-python instead: `legacy-custom-id-reference.json` (20 rows, blob `dc555646`), asserted by `legacy-id-tolerance` for the historical-id lookup (CID-3/CID-4). Its blob is also its exemption key in `no-raw-invisibles`, so a local edit loses the exemption.

**`contract-fixture/` is the API double every SDK in the fleet grades itself against** (spec CONF-2). It is owned here and vendored by other lanes at its git tree id, so a change to it is a contract change for all of them. It mirrors the backend's own code — status codes, middleware order, the write decision, registration's skip rules, the hint acceptance chain — so change it only against that code, never to make a test pass. It has no request log by design: a test observes a write only as state read back, which is what CONF-1 requires. `contract-fixture` is its self-test, and it is mutation-checked: each "says no" case is paired with the condition that permits the same request, so a double that refused everything fails as surely as one that accepted everything. `tests/helpers/contract-fixture.ts` starts it for a test file. After changing it, run its battery in the isolated copy, then announce the new tree id through the Reviewer.

**Contract-tier tests drive the real SDK against the double.** `contract-write-lane` and `contract-hint-lane` run `LangsysApp.init` with real `fetch` against it, through `tests/helpers/sdk-session.ts`, and assert only on the double's accepted state or on what `t()` renders. An absence is evidence only where the double would have accepted the action: it refuses a read-only session's write and declines a writer's hint on its own, so "the SDK did not send" and "the SDK sent and was refused" look identical. Those cases use capability drift — the SDK learns it must not act, then the double is re-seeded so it would accept — and each carries a control showing the double stores the same action from a session that may act. A "never attempt" clause the double cannot see at all is proven at the transport seam instead, in `never-attempt`. The session helper resets the singleton's state between sessions; a new piece of per-session state in `Translations` belongs in its reset.

**Mutation batteries run in an isolated copy, never in this working tree.** The bindings and JS Server load this repo through a symlink, so a battery that rewrites `src/` or rebuilds `dist` in place voids every consumer's run for as long as it lasts, even when each restore is byte-exact. Copy the tree (`rsync -a --exclude node_modules --exclude .git --exclude dist`), symlink `node_modules` into the copy, then build and mutate there. Fleet norm. **Guard the copy path**: if the sync fails, abort — do not let `cd` fall through. A battery once ran in the working tree because the scratchpad script had been cleaned away and the failure was not checked.
