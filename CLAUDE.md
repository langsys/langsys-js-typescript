...
`npm test` runs vitest over `tests/` — 34 files, 620 tests (`api`, `api-reachability`, `cache-scope`, `canonicalization-agreement`, `catalog-envelope`, `changelog-completeness`, `conformance-structure`, `content-block-identity`, `custom-id`, `custom-id-cross-impl`, `discovery`, `ellipsis-warning`, `grant-lane`, `init-settle`, `interpolate`, `interpolation-cross-impl`, `langsys-app`, `locale`, `logger`, `no-private-fields`, `no-raw-invisibles`, `obs-notice`, `persist`, `pure-subpath`, `rich-phrase-identity`, `richtext`, `seed-catalog`, `ssr-strategy-isolation`, `tfunction-identity`, `tokenizer-convergence`, `tokenizer-cross-impl`, `translate`, `translations`, `write-lane`).
