...
- **`langsys-js-typescript/pure` — a side-effect-free subpath for servers and workers.** Exports the identity contract with no DOM: `generateCustomId`, `generateLegacyCustomId`, `canonicalContentBlockJson`, `canonicalizeLocale`, `maximizedLangScript`, `interpolate`, `defaultedArguments`, `isICU`, `normalizeMarkupPlaceholders`, `normalizeTokenText`, `stripC0Controls`, `findUnusedParamKeys`, `encodeRichPhrase`, `md5`, `md5Legacy`, `isEmpty`, `TRANSLATABLE_ATTRIBUTES`, `NON_TRANSLATABLE_ELEMENTS`, plus the markers in both spellings — `PHRASE_MARKER_ATTR`, `PHRASE_MARKER_ATTR_LEGACY`, `PHRASE_MARKER_ATTRS`, `CONTENT_BLOCK_MARKER_ATTR`, `CONTENT_BLOCK_MARKER_ATTR_LEGACY`, `CONTENT_BLOCK_MARKER_ATTRS`, `RESOLVED_MARKER_ATTR`, `RESOLVED_MARKER_ATTR_LEGACY`, `RESOLVED_MARKER_ATTRS`, the server-message pieces `templateMarkers`, `fillTemplate`, `resolveServerMessages`, `toServerMessage` and `DEFAULT_SERVER_MESSAGE_CATEGORY`, and the legacy-key pieces `convertLegacyValue`, `convertLegacyPluralForms`, `convertLegacyCall`, `createLegacyKeys`, `LegacyFormatError`, `LEGACY_FORMATS` and `SUPPORTED_LEGACY_FORMATS`, and the snapshot pieces `buildSnapshot`, `parseSnapshot`, `canonicalSnapshotJson`, `snapshotChecksum`, `SnapshotError`, `SNAPSHOT_FORMAT` and `SNAPSHOT_VERSION`. Forty-six in all, and they are named rather than summarised because a consumer reading this is deciding what to import. Nothing it exposes touches `window`, `document`, `navigator` or storage, at import time or on any call path. The DOM tokenizer itself is **not** exported — `tokenizeElement` walks real nodes and reads computed styles — but everything it is built from is, so a server doing its own parsing can produce byte-identical ids without reimplementing a rule.

  **`encodeRichPhrase` is the exception to that split, and the reason is worth stating.** For a content block the WALK is identity — token arity and order — so a server reproduces it from the exported rules. For `<Phrase>` only the assembled string is identity, so the whole encoder is exported instead, generic over the host's node type: pass your own nodes, get the phrase string and your node handles back in marker order. `encodeRichText` (the DOM entry) is now a node-shape mapping over it, so there is one implementation of a string that is a catalog key rather than two that agree until one changes.
...
- **A host with its own logger receives `interpolate`'s notices.** A server with a per-instance debug setting could not reach the core's process-wide logger, so the notice for a defaulted ICU argument never fired through it. `interpolate(template, params, locale, { onDefaulted, onFormatterFailure })` now passes each notice to the caller's handler, on every occurrence, and the core logger stays silent for it: the caller applies its own debug switch and deduplication. `onDefaulted` receives the defaulted argument names, the template and the locale; `onFormatterFailure` receives the formatter's error, the template and the locale. `defaultedArguments(template, params, locale?)` returns the names a render would default, from the same recovery walk, for a host that emits the notice itself. Without options, behaviour is unchanged.

- **Catalog snapshots: a first render or an offline session with no network call.** `LangsysApp.loadSnapshot(snapshot, locale?)` loads a `langsys-catalog-snapshot` file, exported by any Langsys SDK, synchronously as the preloaded catalog: `t()` returns its translations on the next line. The locale defaults to the user's, then the snapshot's base locale. The snapshot is a cache, not the catalog of record: `init()` still fetches the catalog, which replaces it and supplies anything it lacks. Registration is decided only against that live catalog: a phrase the snapshot holds is still queued, and the queue waits for the live catalog before anything is sent or dropped. With no network the snapshot keeps rendering, and a phrase it lacks shows its source text. A snapshot is refused with a `SnapshotError` naming the reason: a different format, an unsupported version, a missing member, or a checksum its contents no longer match, which is how an edited file is caught. `buildSnapshot` writes the format, for an export tool. The checksum is SHA-256 over a canonical serialisation every SDK computes byte for byte, and the fleet's shared `snapshot-vectors.json` is authored here.

- **Legacy-key migration: keep your i18n keys and your source-language file, and delete the rest.** Pass the kept files to `init` as `legacyKeys: [{ name, format?, namespace?, data }]` and `t()` resolves its argument as a key first. A key's value is the registered phrase, never the key: `{{name}}`, `:name`, `%{name}` and `%(name)s` become `{name}`, and a plural becomes an ICU plural by the file's format. `vue-i18n` pipes select by exact count, `i18next` suffix pairs (`items_one`/`items_other`) by CLDR category. A path-shaped key's first segment, or the file's `namespace`, is the category unless the call passes one. A key no file holds is literal source text, noted at debug. A value the conversion does not recognise, such as a `|` in a `plain` file or `:Name`, registers exactly as written and warns. Formats are `i18next`, `vue-i18n` and `plain` (the default for a file that declares none). A file in any other format (`laravel`, `rails-i18n`, `gettext`, or a `.php`, `.yml`, `.po` or `.mo` name) makes `init` throw `LegacyFormatError`, naming the format and the file. The mode is off unless configured. The conversion and resolver are exported from `/pure` as well, so a server `t()` converts through the same code. The fleet's shared `mig-vectors.json` is authored here.

- **Server messages: render a framework's own errors translated, without changing its response.** A Langsys server SDK attaches entries beside the framework's native error body. Each entry carries `template`, the framework's sentence with the field's label written in and `{name}` markers, the `params` that fill it, `message` (the filled text), and the framework's own `field` and `code`, unchanged. `resolveServerMessages(body, { key })` reads the entries at the configured key; `pieces` maps other piece names and `resolver` builds entries from the body. The body is never searched by shape and never changed, and with neither `key` nor `resolver` the function throws. `renderServerMessage(entry)` renders the template through `t()` under the messages category and shows `message` when the catalog has no translation. It never looks up `message`, and an entry with no template shows its `message`. The category is `Errors` unless `messagesCategory` is set in `init`, and must match the server's. `templateMarkers` and `fillTemplate` implement the server's `{name}` marker grammar. The fleet's shared `server-message-vectors.json` is authored here from real Laravel, Pydantic, Django, DRF and ActiveModel messages.

- **`setTeardownSignal(subscribe)`: flush on hosts with no `document`.** In a browser the SDK sends what is still queued when the page is hidden or unloaded. React Native has no `document` to listen to, so anything queued when the app left the foreground was lost. The host now supplies the signal: `subscribe` receives a `fire` function to call as the app goes away, and returns an unsubscribe. Replacing the signal, or passing `null`, releases the previous one first, so Fast Refresh never flushes twice. Same shape as `setPersistStorage`; the React Native binding wires it to `AppState`.

- **`notifyNavigation()`: tell the SDK the route changed.** Content that stays mounted across a client-side route change — a layout, a header — was never looked up again, so a phrase missing from it was reported for the first page only. After the call, anything bound to `tSignal` re-renders and each miss is recorded at the new URL; `Translate` and `Phrase` instances re-run their lookup while their element is still on the page. Also on `LangsysApp`. Framework bindings call it from their router's after-navigation hook.
...
- **Registration and discovery can be limited to the base locale, from a project setting.** A server SDK renders a page in the visitor's language, and the catalog is keyed by source text, so every string on that page is a translation: a write-enabled session browsing in Spanish filed Spanish sentences as new source phrases, and a read-only one reported the localized URL for discovery. With `discovery_base_locale_only` on for the project, a miss is recorded only while the loaded locale is the base locale, on both lanes. It arrives in the handshake — on the authorize response and on every catalog fetch — and there is no client override, so a project has one answer. Absent means off, which is what every existing project gets. The trade-off, and why it is off by default: an English fallback appearing on a localized page is no longer discovered from that page.

- **`data-ls-resolved`: text a server already translated is never registered.** A producer marks a scope it resolved — `<html data-ls-resolved="es-es">` is the usual shape — and the client records nothing for text inside it, neither a registration nor a discovery report. Inherited, so the nearest marked ancestor decides and a subtree opts back out with `="false"` or `="0"`; `data-langsys-resolved` is accepted on read. It says nothing about identity: a content block inside a resolved scope keeps its id and still renders a translation the catalog holds. Agreed with the `langsys-php` and Laravel packages, which write it.
...
- **C0 control characters are removed from phrases before whitespace collapses, and ids containing one move.** The 28 characters U+0001–U+0008, U+000B, U+000C and U+000E–U+001F are deleted, not turned into a space, from text nodes, attribute values, `<Phrase>` strings and `t()` keys, on registration and on lookup alike. So `A`+U+001C+`long description` registers as `Along description`. Vertical tab and form feed are in that set: they used to collapse to a space and are now removed. Tab, line feed and carriage return are not in it and still collapse. NUL, DEL and the C1 range are kept. `t()` still renders the phrase it was given, so only the key changes. `stripC0Controls` is exported from `/pure`.

  **Blast radius.** Only blocks and phrases containing one of these characters move. A browser DOM keeps them, while a `libxml2` older than 2.14 drops them from text, so a server rendering through such a parser already produced the stripped id for text. A moved entry re-registers under its new id and shows source text until re-translated. Its old entry remains as an orphan.
...
- **The tokenizer converges with `langsys-php`, and this changes `custom_id` for affected blocks.** Those blocks re-register under the new id and show source text until re-translated; their previous catalog entries remain as orphans. Tolerance for these moved ids was considered and deliberately not added, so there is no migration — re-registration is the accepted path. The historical-id lookup added in this release is a different thing: it covers older hash shapes, and a block whose tokens changed below still re-registers. What changed:
  ...
  - **`<script>`, `<style>`, `<template>`, `<noscript>` and `<math>` contents are no longer harvested.** They were: `<style>.plan{color:#fff}</style>` registered `.plan{color:#fff}` as a translatable phrase and `<script>window.dataLayer.push(1)</script>` registered the statement, both then sent for machine translation.
    ...
    **`<math>` joined the list last**, and it is the sharpest of them: `<p>Area <math><mi>x</mi><mo>+</mo><mn>2</mn></math> units</p>` tokenized to `['Area','x','+','2','units']`, registering a bare variable and an *operator* as translatable phrases. Harvesting CSS wasted money on a string nobody read; translating `+` corrupts the expression. **`<svg>` is deliberately NOT excluded** — an `<svg><text>` renders visible words, so its text is translated, and an inline icon never causes its parent block to lose the parent's own text.

    **`<noscript>` content no longer contributes tokens.** It was harvested in an earlier revision of this change, on the reasoning that its text shows to a real visitor when scripting is off. That premise is true but does not survive two questions: with scripting off a browser SDK is not running, so it can translate nothing on that page anyway; and with scripting *enabled* — the HTML spec's default — a parser treats the body as raw text, so the token is the markup string `<p>Enable JavaScript</p>` rather than the sentence. Registering that sends markup for translation, which is what this change exists to stop.
...
- **A phrase the message formatter cannot render still renders, and says so.** When `intl-messageformat` throws on a phrase that parses and is not missing an argument, the SDK renders it itself. For each `select` or `plural` it takes the branch for the supplied value: an exact `=N` first, then the value's CLDR plural category in the render locale, then `other`. It fills in the supplied values and shows an unsupplied one as `{argName}`, so the result is never empty and never raw ICU source. A warning names the phrase, the locale and the formatter's error. It fires even with debug logging off, because the phrase needs fixing, and it fires once per phrase and locale. A missing argument is still recovered silently, with only the debug-level notice.
...
- **The base-locale discovery gate now covers content blocks.** With `discovery_base_locale_only` on, a content block in a non-base locale was still registered, or reported for discovery, because the gate was applied only to `t()` misses. It now applies to `registerContentBlock` as well.

- **A failed catalog fetch is not repeated on every locale emission.** After a catalog fetch fails for a project and locale, lookups for that pair render source text for a short window without fetching again: 3 seconds, doubling on each consecutive failure to at most 5 minutes, and cleared by the first success. Concurrent fetches for the same pair now share one request. An explicit `LangsysApp.refresh()` still fetches inside the window.

- **Re-initialising with a different key no longer warns about the previous one.** On a second `init()`, the write-capability notice read the key type of the previous initialisation, so a session with a read key could be warned that its `ip_write` key cannot write.
