...
- **Server messages: find the entries in an error response and render them translated.** `resolveServerMessages(body, options?)` returns every `{ field?, code, message, template, params? }` entry in a response body, wherever the app's envelope puts it, with `key` to look in one place and `resolver` to map another shape. `renderServerMessage(entry)` renders the entry's template through `t()` under the messages category, and shows the entry's `message` when the catalog has no translation; it never looks up `message`. The category is `Errors` unless `messagesCategory` is set in `init`, and must match the server's. `templateMarkers` and `fillTemplate` implement the server's `{name}` marker grammar, and `SERVER_MESSAGE_CODES` lists the shared validation codes. The fleet's shared `server-message-vectors.json` is authored here.
