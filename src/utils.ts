export const isEmpty = (data: unknown): boolean => {
    ...
    if (typeof (data as { length?: number }).length !== 'undefined') {
...
export function structuredCloneShim<T>(obj: T): T {
    if (typeof structuredClone === 'function') return structuredClone(obj);

    const clone = <V>(o: V): V => {
        if (typeof o !== 'object' || o === null) return o;
        if (Array.isArray(o)) return o.map((item) => clone(item)) as unknown as V;
        const out: Record<string, unknown> = {};
        for (const key in o as object) {
            if (Object.prototype.hasOwnProperty.call(o, key)) {
                out[key] = clone((o as Record<string, unknown>)[key]);
            }
        }
        return out as V;
    };
